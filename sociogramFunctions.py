...
def __calcSizes__(G,dictUserSize,scaleUp):
...
def removeUsers(df,removalList,columnName=None,strict=False):
def keepOnlyUsers(df,keepList,columnName=None,strict=False):
def listIsolates(df,columnName,exceptedList=None):
def listTopCentralityUsers(df,columnName,numUsers,centralityType='degree'):
def drawGraph(df,columnName,labelList=None,colorDict=None,sizesDict=None,sizeScale=100,edgesDict=None,widthScale=1):
...
def assignCommunityColors(communitiesDict,maxCommNum=None):
	...
	cMapping = cm.get_cmap('hsv',numGroups)
	colorList = cMapping(range(numGroups))
...
def drawGraph(df,columnName,title=None,removeIsolates=False,labelList=None,colorDict=None,minSize=20,sizesDict=None,sizeScale=100,edgesDict=None,widthScale=1,edgeAlpha=0.5,block=True):
