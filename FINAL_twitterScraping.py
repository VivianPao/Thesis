...
def findFollowing(username,limitNum):
	c = twint.Config()
	c.Limit = limitNum
	c.Pandas = True
	c.Username = username
...
if __name__ == "__main__":
	df = scrapeTopic("badgerys creek airport",3,["2020-01-01","2020-05-02"])
	df.to_csv("test.csv")

	# followerDf = pd.DataFrame()
	# for user in userList:
	# 	print('Searching: ',user)
	# 	data = findFollowing(user)
	# 	followerDf = pd.concat([followerDf,data])

	# followerDf.to_csv('followers_data.csv')
	# print(followerDf)
